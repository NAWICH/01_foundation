...
from app.database import load_posts, save_posts, find_post_by_id,get_post_by_sentiment,find_user_by_email,find_user_by_username
from app.models import UserLogin, UserRegister, PostUpdate, PostCreate, Token, AnalyzeRequest, SentimentAnalysis
from app.auth import get_current_user
from app.ml_service import analyze_sentiment
load_dotenv()
app = FastAPI()
...
@app.get("/api/posts")
async def get_all_posts(sentiment: Optional[str] = None):
    """Get all posts, Optionally filtered by Sentiment"""
    if sentiment:
        #Very Negative, Negative, Neutral, Positive, Very Positive)
        sentiment_upper = sentiment.upper()
        if sentiment_upper not in ['VERY NEGATIVE, NEGATIVE, NEUTRAL, POSITIVE, VERY POSITIVE']:
            raise HTTPException(status_code=400, detail="Invalid sentiment value")
        posts = get_post_by_sentiment(sentiment_upper)
    else:
        posts = load_posts()
...
async def create_post(body: PostCreate, current_user: dict = Depends(get_current_user)):
    ...
    sentiment_result = analyze_sentiment(body.content)
    new_post = {
        "title" : body.title,
